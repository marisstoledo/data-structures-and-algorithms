...
const maxInArray = (arr) => {
 return arr.reduce((accumaltor, currentValue) =>{
   return currentValue > accumaltor ? currentValue : accumaltor;
 })
};
...
const checkValues = (obj, value) => {
  ...
  Object.values(obj).forEach(str => str === value ? newBoolean = true: false);
  return newBoolean;
};
...
const getHouses = (arr) => {
  let houses = [];
  Object.values(arr).forEach(item=> houses.push(item.house));
  return houses;
};
